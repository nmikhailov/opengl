...
GLuint TextureManager::loadTexture(const QImage &img, TextureInfo::T_TYPE type) {
    ...
    if (type == TextureInfo::T_2D) {
        glTexImage2D(internal, 0, GL_RGBA, image.width(), image.height(), 0, GL_BGRA, GL_UNSIGNED_BYTE, image.mirrored(false, true).bits());
        ...
        for (int i = 0; i < 6; i++) {
            glTexImage2D(key[i], 0, GL_RGBA, images[val[i]].width(), images[val[i]].height(), 0, GL_BGRA, GL_UNSIGNED_BYTE, images[val[i]].bits());
        }
    }
